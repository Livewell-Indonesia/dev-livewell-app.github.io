  ...
  List<Widget> buildDots() {
    ...
    for (int i = 0; i < widget.number; i++) {
      if (widget.size == DotSize.small) {
        radius = widget.random.nextDouble() * 2 + 2;
...
class DotPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // }
    var dy =
        size.height * (initialPosition + distance * fraction) - (radius * 2);
    var dx = (-start - radius + (size.width + diameter + start) * fraction) -
        2 * radius;
    offset = Offset(dx, dy <= 0 ? radius * 2 : dy);
    if (dx >= 21 && dx <= size.width - 20) {
