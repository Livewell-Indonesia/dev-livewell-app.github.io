...
class UpdateWeightController extends BaseController {
  ...
  void getMealHistory() async {
    GetUserMealHistory getUserMealHistory = GetUserMealHistory.instance();
    var currentDate = DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day, 23, 59, 59);
    var endDate = DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day - 90, 0, 0, 0);
  ...
  double getYValue(int index) {
    ...
    if (mealHistoryList.isNotEmpty) {
      ...
      for (var data in mealHistoryList) {
        if (DateTime.parse(data.mealAt!).year == date.year && DateTime.parse(data.mealAt!).month == date.month && DateTime.parse(data.mealAt!).day == date.day) {
    ...
    if (Get.isRegistered<DashboardController>()) {
      ...
      result.fold((l) {}, (r) {
        ...
        if (weightHistory.isNotEmpty) {
          var maxWeight = weightHistory.asMap().entries.reduce((a, b) => a.value.weight! > b.value.weight! ? a : b).value.weight!.toDouble();
          minY.value = weightHistory.asMap().entries.reduce((a, b) => a.value.weight! < b.value.weight! ? a : b).value.weight!.toDouble();
          maxY.value = targetWeight.toDouble() > maxWeight ? targetWeight.toDouble() : maxWeight;
  ...
  void onUpdateTapped() async {
    ...
    if (Get.isRegistered<DashboardController>()) {
      EasyLoading.show();
      final result = await updateUserWeight(UpdateWeightParams(weight: inputtedWeight!));
      EasyLoading.dismiss();
      result.fold((l) {}, (r) {
  ...
  void onUpdateTargetTapped() async {
    ...
    var params = QuestionnaireParams.asParams(
      ...
      DateFormat('yyyy-MM-dd').format(DateTime.parse(dashboardController.user.value.birthDate!)),
      ...
      dashboardController.user.value.onboardingQuestionnaire!.glassesOfWaterDaily!,
      dashboardController.user.value.onboardingQuestionnaire!.sleepDuration!,
      dashboardController.user.value.onboardingQuestionnaire!.dietaryRestrictions!.first,
